...
if(count($_POST) == 4) {
    ...
    $dirs = require_once $path . 'libraries/configuration/dirs.php';

    foreach($dirs as $access => $catalogs) {
    ...
    $files = require_once $path . 'libraries/configuration/files.php';
    ...
    $tables = require_once $path . 'libraries/configuration/tables.php';

    foreach($tables as $table) $descriptor->exec($table);

    // Загрузка контента

    $content = require_once $path . 'libraries/configuration/content.php';

    foreach($content as $insert) $descriptor->exec($insert);

    // Загрузка объектов

    if(is_dir($path . 'libraries/configuration/objects')) {

        foreach(glob($path . 'libraries/configuration/objects/*') as $file) {
