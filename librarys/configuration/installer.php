...
if(count($_POST) == 4) {
    ...
    $dirs = require_once $path . 'librarys/configuration/dirs.php';
    ...
    $files = require_once $path . 'librarys/configuration/files.php';
    ...
    $tables = require_once $path . 'librarys/configuration/tables.php';

    foreach($tables as $table) $descriptor->exec($table);

    // Загрузка контента

    $content = require_once $path . 'librarys/configuration/content.php';

    foreach($content as $insert) $descriptor->exec($insert);

    // Загрузка объектов

    if(is_dir($path . 'librarys/configuration/objects')) {

        foreach(glob($path . 'librarys/configuration/objects/*') as $file) {
